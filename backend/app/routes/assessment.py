...
from app.schemas.assessment import AssessmentBase, AssessmentResponse

router = APIRouter(prefix="/api/v1/assessments", tags = ["Assessments"])

@router.post("/create", response_model= ApiResponse[AssessmentResponse])
async def creare_assessment_result(assessment_data: AssessmentBase, db: Session = Depends(get_db), current_user: int = Depends(auth_dependency)):
    try:
